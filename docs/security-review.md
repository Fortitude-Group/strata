...
remaining exposure is *defence-in-depth*: bounds safety currently depends on the CLR's own array-bounds
...
is a hardening change, not a fix for an observed unsafe read. It is appropriate for a follow-up, not urgent.
...
entirely of short call-like x86-64 encodings decodes to on the order of 1 to 3 million instructions;
...
- **Upload cap: present and enforced twice.** `DemoOptions.MaxUploadBytes` defaults to 8 MiB.
  ...
  lies about `Size` cannot stream more than the cap. Good defence in depth. **Info.**
...
## (f) Supply chain: pinned dependencies and licence posture
...
**One item this review could not independently verify:** the *current* licence terms of each pinned
...
self-consistent, not a fresh third-party licence audit. All six are commonly permissive
...
licence/vulnerability check (`dotnet list package --vulnerable`, or a dedicated SCA tool such as
...
| 8 | (f) supply chain | info/low | Pins are exact and consistent; licence terms not independently re-verified in this pass; native Capstone binary pin worth a release-time check |
