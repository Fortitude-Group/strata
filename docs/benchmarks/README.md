...
| Version-resolution accuracy (correct minor) | **100%** | n/a (Checkpoint B: ≥ 70%) | ✅ |
| Scan wall-time (4 binaries) | ~0.5 s | n/a | n/a |
...
- On the hardest cross-optimisation case (**`-O0` target vs `-O2/-O3/-Os` corpus**), function-level
  MinHash matching does **not** fire, because the instruction streams differ too much, so identification here
...
linearly, a 40 MB binary lands at roughly 5 to 6 s, far inside the **< 5 min** budget (SC-005). The
